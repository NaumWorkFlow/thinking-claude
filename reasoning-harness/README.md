...
## Команды
- `/max` — Форсаж: снять экономный режим, Активационная карта, полный пайплайн.
- `/test-module <имя>` — A/B-проверка модуля (нужен ли он, не вредит ли).
- `/harness-probe` — регресс-пробы после смены поколения модели (10 канонических ловушек).
- `/harness-maintenance` — обслуживание: гигиена памяти, реестр гипотез, консистентность документов.
